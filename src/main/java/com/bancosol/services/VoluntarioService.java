...
public class VoluntarioService {
    ...
    //AYUDA IA
    //carga los turnos del vol y los mete al dto, se hcae asi para evitar q jpa entre en un bucle infinito (Voluntario -> TiendaTurno -> Voluntario...)
    ...
    private void procesarAsignaciones(Voluntario voluntario, List<TurnoNuevoDTO> turnos, Long campaniaId) {
        ...
        for (TurnoNuevoDTO tDto : turnos) {
            ...
            if (tienda == null) continue; // si no hay tienda pasamos al sig
            ...
            Turno turno = turnoRepo.findByDiaAndFranjaHorariaAndCampania_Id(diaEnum, franjaEnum, campaniaId)
                    .orElseGet(() -> turnoRepo.save(Turno.builder().dia(diaEnum).franjaHoraria(franjaEnum).campania(campania).build()));
    ...
    public void actualizarVoluntario(Long voluntarioId, VoluntarioNuevoDTO dto) {
        Voluntario v = voluntarioRepo.findById(voluntarioId).orElseThrow(() -> new RuntimeException("Voluntario no encontrado"));
