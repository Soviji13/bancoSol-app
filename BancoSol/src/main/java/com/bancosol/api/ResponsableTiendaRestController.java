...
public class ResponsableTiendaRestController {
    ...
    public ResponsableTiendaRestController(ResponsableTiendaService service) {
        this.service = service;
    }

    @GetMapping
    public ResponseEntity<List<ResponsableTiendaDTO>> getAll() {
        return ResponseEntity.ok(service.listarTodos());
    }

    @PostMapping
    public ResponseEntity<ResponsableTiendaDTO> crear(@RequestBody ResponsableTiendaNuevoDTO dto) {
        return ResponseEntity.ok(service.crearResponsable(dto));
    }
}